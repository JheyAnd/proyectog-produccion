...
function checkIsPatioSur(id?: string): boolean {
  ...
  return normalized === 'patiosuroe1035' || normalized === 'oe1035';
}
...
export default function ProjectGuard() {
  ...
  // ✅ CRITICAL: Si es Patio Sur o Lyra, SIEMPRE deja pasar sin verificaciones
  // Esto asegura que los módulos funcionen aunque falle la API
  const isPatioSur = checkIsPatioSur(projectId);
  const isLyra = checkIsLyra(projectId);

  const proyectoConfigurado = useMemo(() => {
    if (isPatioSur || isLyra) return true;
  ...
  }, [bcStatus, isPatioSur, isLyra]);

  const isActiveProject = isPatioSur || isLyra || proyectoConfigurado;
