    ...
    <div className="rounded-xl border border-steel-200 dark:border-steel-700 bg-white dark:bg-steel-800 shadow-lg px-4 py-3 text-xs min-w-[220px] pointer-events-none">
      ...
      <div className="space-y-1.5">
        {payload.map((p) => {
          ...
          // Mostrar solo los 4 indicadores solicitados + Valor Pagado
          const allowedNames = ['Ingreso', 'Egreso', 'Valor Pagado', 'FCN', 'FCA (Acumulado)'];
...
const LEGEND_ITEMS = [
  ...
  { key: 'Valor Pagado', color: '#eab308', type: 'bar' },
...
export default function CashFlowChart({ data, onMonthClick }: CashFlowChartProps) {
  ...
  const chartData = data.map((d) => ({
    ...
    'Ingreso': d.income,
    'Egreso': -d.expense,
    'Valor Pagado': d.paid ? -Math.abs(d.paid) : 0,
    'FCN': d.net,
