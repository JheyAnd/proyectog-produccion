...
async def delete_project(
    ...
    Reglas:
    1. Solo administradores y gerentes.
    ...
    # Nota: activity_logs está en la lista para limpiar historial previo, 
    # pero el registro de eliminación actual se mantiene si se inserta después o si no se filtra en el DELETE.
    # Para asegurar que el LOG DE ELIMINACIÓN sobrevive, lo manejamos con cuidado.
    
    for tabla in TABLAS_CASCADE:
        try:
            # Si es activity_logs, solo borramos los registros ANTERIORES al backup de recién
            if tabla == 'activity_logs':
                await db.execute(
            ...
            pass  # tabla puede no tener registros o la columna project_id — continuar
