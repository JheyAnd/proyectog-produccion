...
class BusinessCaseResponse(BaseModel):
    ...
    venta_suministro: Optional[float] = None
    ...
    costo_suministro: Optional[float] = None
...
async def update_chapter(
    ...
    if project_id == 'patio-sur-oe1035':
        raise HTTPException(status_code=403, detail="Patio Sur está protegido. Operación rechazada.")
    bc = await _get_business_case(db, project_id)
...
async def import_excel(
    ...
    if project_id == 'patio-sur-oe1035':
        raise HTTPException(status_code=403, detail="Patio Sur ya tiene datos cargados. Contacta a un administrador si necesitas modificarlos.")
...
async def import_lyra_carsan(
    ...
    if project_id == 'patio-sur-oe1035':
        raise HTTPException(status_code=403, detail="Patio Sur no permite importaciones mediante este endpoint.")
...
PROYECTOS_PROTEGIDOS = ['patio-sur-oe1035']
PROYECTOS_SIN_PARSER_GENERICO = ['patio-sur-oe1035', 'lyra-carsan-oe2000']
...
async def get_business_case_status(
    ...
    return {
        ...
        "venta_suministro": bc.venta_suministro,
        ...
        "costo_suministro": bc.costo_suministro,
...
async def parse_business_case_detail(
    ...
    # Protección
    if project_id in ('patio-sur-oe1035', 'lyra-carsan-oe2000'):
