...
export function formatCurrency(v: number | string | null | undefined): string {
  ...
  if (typeof v === 'string') {
    // Si ya tiene símbolos de moneda, lo dejamos así
    if (v.includes('$')) return v;
    const p = parseFloat(v.replace(/[^0-9.-]/g, ''));
    if (isNaN(p)) return v;
    v = p;
  }
  return new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 }).format(v as number);
}

export function formatPercent(v: number | string | null | undefined): string {
  if (v == null || v === '') return '—';
  
  // Si es un string que ya parece formateado, lo devolvemos tal cual
  if (typeof v === 'string') {
    if (v.includes('%')) return v;
  ...
  // Si es <= 1 interpretamos como fracción (0.59 = 59%)
  const num = v as number;
  const pct = num <= 1 && num >= 0 ? num * 100 : num;
  const formatted = pct % 1 === 0 ? pct.toFixed(1) : parseFloat(pct.toFixed(2)).toString();
  return `${formatted}%`;
}

// ── Determinar estado visual a partir del avance ──
export function projectStatus(p: ProjectTracking): 'completado' | 'en_progreso' | 'atrasado' | 'sin_datos' {
