...
export async function exportProjectsToExcel({ projects, userName, filtersUsed, suggestedFilename }: ExportOptions) {
  ...
  projects.forEach((p, index) => {
    ...
    const getVal = (val: any) => {
      if (val === null || val === undefined || val === '' || val === '—') return 'sin datos';
      return val;
    };
    ...
    const row = worksheet.addRow({
      idx: index + 1,
      nombre: getVal(p.nombre_proyecto || p.sheet_name),
      codigo: getVal(p.codigo_proyecto),
      cliente: getVal(p.cliente),
      director: getVal(p.director_proyectos),
      residente: getVal(p.ingeniero_residente),
      supervisor_col: getVal(p.supervisor),
      
      fecha_terminacion_estimada: getVal(p.fecha_terminacion_estimada),
      ...
      estado: getVal(status.replace('_', ' ').toUpperCase()),
    });
    ...
    moneyCols.forEach(col => {
      ...
      if (cellVal !== null && cellVal !== undefined && cellVal !== 'sin datos' && typeof cellVal === 'number') {
        row.getCell(col).numFmt = '"$"#,##0';
      }
    });

    const progressVal1 = row.getCell('avance_prog').value;
    if (progressVal1 !== null && progressVal1 !== undefined && progressVal1 !== 'sin datos' && typeof progressVal1 === 'number') {
      row.getCell('avance_prog').numFmt = '0.00%';
    }
    const progressVal2 = row.getCell('avance_real').value;
    if (progressVal2 !== null && progressVal2 !== undefined && progressVal2 !== 'sin datos' && typeof progressVal2 === 'number') {
      row.getCell('avance_real').numFmt = '0.00%';
    }
