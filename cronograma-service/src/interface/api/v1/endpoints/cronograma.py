...
async def import_cronograma(
    ...
    try:
        ...
        import datetime
        # Eliminar el cronograma_obra anterior
        await db.execute(
            text("DELETE FROM entregables WHERE project_id = :pid AND doc_type IN ('cronograma', 'cronograma_obra')"),
            {"pid": project_id}
        )
        record = EntregableModel(
            project_id=project_id,
            doc_type="cronograma_obra",
            filename=file.filename or "cronograma.xlsx",
...
from src.infrastructure.database.models.cronograma_simulacion_model import CronogramaSimulacion
...
@router.get("/{project_id}/simulaciones")
async def list_simulaciones(project_id: str, db: AsyncSession = Depends(get_db_session)):
    stmt = select(CronogramaSimulacion).where(CronogramaSimulacion.project_id == project_id).order_by(CronogramaSimulacion.created_at.desc())
    result = await db.execute(stmt)
    ...
    return [
        {
            ...
            "nombre": s.nombre,
            "estado_json": json.loads(s.estado_json),
            "created_at": s.created_at
        } for s in sims
    ]

@router.post("/{project_id}/simulaciones")
async def create_simulacion(project_id: str, data: SimulacionCreate, db: AsyncSession = Depends(get_db_session)):
    sim = CronogramaSimulacion(
        project_id=project_id,
        nombre=data.nombre,
        estado_json=data.estado_json
    )
    db.add(sim)
    await db.commit()
    await db.refresh(sim)
    return {"id": sim.id, "nombre": sim.nombre}

class SimulacionUpdate(BaseModel):
    nombre: Optional[str] = None
    estado_json: Optional[str] = None

@router.put("/simulaciones/{sim_id}")
async def update_simulacion(sim_id: int, data: SimulacionUpdate, db: AsyncSession = Depends(get_db_session)):
    stmt = select(CronogramaSimulacion).where(CronogramaSimulacion.id == sim_id)
    result = await db.execute(stmt)
    ...
    if data.estado_json is not None:
        sim.estado_json = data.estado_json
    await db.commit()
    await db.refresh(sim)
    return {
        "id": sim.id,
        "nombre": sim.nombre,
        "estado_json": json.loads(sim.estado_json) if sim.estado_json else {}
    }

@router.delete("/simulaciones/{sim_id}")
async def delete_simulacion(sim_id: int, db: AsyncSession = Depends(get_db_session)):
    stmt = select(CronogramaSimulacion).where(CronogramaSimulacion.id == sim_id)
    result = await db.execute(stmt)
    sim = result.scalar_one_or_none()
    if not sim:
        raise HTTPException(404, "Simulacion no encontrada")
    await db.delete(sim)
    await db.commit()
    return {"ok": True}

# --- S-Curve Simulation and Forecasting Engine Integration ---
from src.application.services.simulation_engine import UniversalScheduleInput, SimulationEngine, SimulationResult
from typing import Dict, Optional

class RunSimulationRequest(BaseModel):
    schedule_data: UniversalScheduleInput
    current_period_index: int
    custom_progress: Optional[Dict[str, float]] = None

@router.post("/simulaciones/run", response_model=SimulationResult)
async def run_simulation_engine(data: RunSimulationRequest):
    """
    Runs S-curve simulation and SPI projections in an in-memory sandbox.
    Does not persist or modify live database records.
    """
    try:
        engine = SimulationEngine(data.schedule_data)
        result = engine.run_simulation(
            current_period_index=data.current_period_index,
            custom_progress=data.custom_progress
        )
        return result
