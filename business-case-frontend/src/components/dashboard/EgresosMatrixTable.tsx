...
export default function EgresosMatrixTable({ currentMonth = '2026-04', projectId = 'patio-sur-oe1035' }: Props) {
  ...
  const grupoData = useMemo(() => {
    const grupos: (EgresoGrupo | 'ingreso')[] = ['ingreso', 'materiales', 'mano_obra', 'administracion', 'intereses'];
  ...
  const exportarExcel = useCallback(async () => {
    ...
    let currentRowIdx = 6;
    const grupos: EgresoGrupo[] = ['ingreso', 'materiales', 'mano_obra', 'administracion', 'intereses'];
  ...
  return (
    <div className="space-y-4">
      ...
      {grupoData.map(({ grupo, items, total }) => {
        ...
        return (
          <div key={grupo} className={clsx('rounded-xl border shadow-sm overflow-hidden bg-white dark:bg-steel-800', cfg.border, cfg.darkBorder)}>
            {/* Header del grupo */}
                          ...
                          <td className="px-3 py-1.5 sticky left-0 bg-white dark:bg-steel-800 hover:bg-steel-50/50 dark:hover:bg-steel-700/30 z-10 border-r border-steel-100 dark:border-steel-700">
                            {editingName === cat.id ? (
                              ...
                              <div className="flex items-center gap-1.5 group">
                                ...
                                <span className={clsx('text-xs font-semibold truncate max-w-[180px]', cfg.color, cfg.darkColor)}>
                                  {cat.nombre}
                                </span>
      ...
      <div className={clsx('rounded-xl border-2 p-3 flex items-center justify-between', saldoOperativo >= 0 ? 'border-emerald-400 bg-emerald-50 dark:bg-emerald-950/30 dark:border-emerald-700' : 'border-red-400 bg-red-50 dark:bg-red-950/30 dark:border-red-700')}>
        <p className="text-sm font-bold text-steel-800 dark:text-steel-100">UTILIDAD</p>
        <p className={clsx('text-lg font-black font-mono', saldoOperativo >= 0 ? 'text-emerald-700' : 'text-red-700')}>
