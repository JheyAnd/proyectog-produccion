...
async def get_current_user(
    ...
    if credentials is None:
        raise HTTPException(
            ...
            detail="Token de autenticacion requerido (No credentials provided)",
    ...
    if payload is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Token invalido o expirado (decode_access_token returned None)",
            headers={"WWW-Authenticate": "Bearer"},
        )
    ...
    if not user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=f"Usuario no encontrado en la base de datos local. Email: {email}, Sub: {payload.get('sub')}",
        )
